...
import { fetchOwnerBookings, type OwnerBooking } from "../lib/bookings";
...
export default function OwnerBookingsClient() {
    ...
    if (bookings.length === 0) {
        return (
            <section className="rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-[0_24px_90px_rgba(249,115,22,0.08)]">
                <h2 className="mb-2 text-xl font-black text-gray-950">No incoming bookings</h2>
                <p className="text-sm text-gray-600">New customer bookings will appear here.</p>
                <Link
                    href="/"
            ...
            {bookings.map((booking) => (
                ...
                >
                    ...
                    <div className="mb-3 rounded-xl border border-gray-100 bg-gray-50 p-3 text-sm text-gray-700">
                        <p>
                            Customer: <strong>{booking.user.firstName || "Guest"}</strong>
                        </p>
